...
if(!isset($_GET['id'])) {
    $response = ['success' => false, 'errorCode' => 'INCOMPLETE_PARAMAETERS', 'errorMessage' => "Parámetros incompletos"];
    echo json_encode($response);
    die;
}
...
if(($cliente = $dao->getClienteByID($id)) === FALSE) {
    $response = ['success' => false, 'errorCode' => 'DB_ERROR', 'errorMessage' => $dao->error];
    echo json_encode($response);
    die;
}

if(($proyectos = $dao->getProyectosByCliente($cliente)) === FALSE) {
    $response = ['success' => false, 'errorCode' => 'DB_ERROR', 'errorMessage' => $dao->error];
    echo json_encode($response);
    die;
}
...
$response = ["success" => true, 'proyectos' => $proyectos];
echo json_encode($response);
